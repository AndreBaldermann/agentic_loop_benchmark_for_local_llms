...
def summary_row(result: LoopRunResult) -> dict[str, Any]:
    ...
    return {
        ...
        "model_call_failures": sum(1 for call in calls if call.call_failed),
        "model_call_error_types": ";".join(sorted({str(call.error_type) for call in calls if call.error_type})),
